...
func (em *httpEmitter) Loop() {
	...
	//err := fasthttp.ListenAndServe(":"+port, handler.HandleFastHTTP)
		...
		if err != nil {
			...
			if err != nil {
				...
				if h.emitter.config.Elo.RecorderFileName != "" {
					...
					h.emitter.recorder.Record(sbuf + end)
					// h.emitter.m.Lock()
					// h.emitter.wbuf.WriteString(sbuf + end)
