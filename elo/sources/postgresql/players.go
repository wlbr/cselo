...
func (s *Postgres) GetPlayerByID(id int64) *elo.Player {
	...
	p := &elo.Player{}
	row := s.db.QueryRow(context.Background(), `SELECT id, initialname, steamid FROM players WHERE id=$1`,
		id)
	err := row.Scan(&p.ID, &p.Name, &p.SteamID)
	switch {
...
func (s *Postgres) GetPlayerBySteamID(steamid string) *elo.Player {
	...
	p := &elo.Player{}
	row := s.db.QueryRow(context.Background(), `SELECT id, initialname, steamid FROM players WHERE steamid=$1`,
		steamid)
	err := row.Scan(&p.ID, &p.Name, &p.SteamID)
	switch {
	case err == pgx.ErrNoRows:
...
func (s *Postgres) GetAllPlayers() elo.PlayersCache {
	...
	rows, err := s.db.Query(context.Background(), "SELECT id, initialname, steamid FROM players;")
	defer rows.Close()
	if err != nil {
	...
	for rows.Next() {
		p := &elo.Player{}
		err = rows.Scan(&p.ID, &p.Name, &p.SteamID)
		switch {
		case err == pgx.ErrNoRows:
